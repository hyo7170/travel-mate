...
//   // 현재 시간을 기준으로 가장 가까운 출발 시간을 가진 항공편을 찾는 함수
//   const findNearestFlight = (flights) => {
//     const currentTime = getCurrentTimeHHMM();
//     let closestFlight = null;
//     let minTimeDifference = Infinity;

//     flights.forEach(flight => {
//       const flightTime = flight.scheduleDateTime.replace(':', '');
//       const timeDifference = parseInt(flightTime, 10) - parseInt(currentTime, 10);
...
//     return airlines.filter(airline => airline.toLowerCase().includes(query.toLowerCase()));
//   };
...
//       <Text style={[styles.cityText, selectedAirline === item && styles.selectedCity]}>{item}</Text>
//     </TouchableOpacity>
//   );
...
//   // 현재 시간에 가장 가까운 출발 시간을 가진 항공편을 찾습니다.
//   const nearestFlight = findNearestFlight(sortedFlights);

//   // 찾은 항공편의 인덱스를 계산합니다.
//   const nearestFlightIndex = sortedFlights.indexOf(nearestFlight);
...
//       <ScrollView style={{ maxHeight: 500 }} ref={scrollRef => {
//         if (scrollRef && nearestFlightIndex !== -1) {
//           const yPosition = nearestFlightIndex * 300; // 예시로 각 항목의 높이를 300으로 가정
//           scrollRef.scrollTo({ y: yPosition, animated: true });
//         }
//       }}>
//         <View style={styles.container}>
//           {sortedFlights.length === 0 ? (
//             <Text style={styles.noFlightsText}>{translate('금일 해당도시 항공편이 없습니다.')}</Text>
//           ) : (
...
//                         {item.flightId} {item.codeshare === "Slave" ? translate('[공동]') : ""}
//                       </Text>
//                     </Text>
//                     <View style={styles.textContainer}>
//                       <View style={{ flex: 1 }}>
//                         <Text style={styles.itemText}>
//                           <Text style={{ fontWeight: 'bold' }}>{translate('도착공항')}</Text>
//                           {'\n'}
...
//                           <Text style={{ fontWeight: 'bold', textAlign: 'right' }}>{translate('출발')}: </Text>
//                           {formatTime(item.scheduleDateTime)}
//                         </Text>
//                         <Text style={[styles.timeText, item.remark === '지연' && styles.delayedTime]}>
...
//                       </Text>
//                       <Text style={styles.inlineItem}>
//                         <Text style={{ fontWeight: 'bold' }}>{translate('체크인')}{'\n'}</Text>
//                         {item.chkinrange}
//                       </Text>
//                       <Text style={[styles.inlineItem, { textAlign: 'center' }]}>
//                         <Text style={{ fontWeight: 'bold' }}>{translate('탑승구')}{'\n'}</Text>
//                         {item.gatenumber}
//                       </Text>
//                     </View>
